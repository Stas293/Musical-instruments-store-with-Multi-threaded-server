...
public class StatusRepository {
    ...
    public synchronized ArrayList<Status> getAllStatuses(@NotNull Connection connection)
        ...
        while (resultSet.next()) {
            statuses.add(new StatusBuilderImpl().setId(Long.parseLong(resultSet.getString("status_id"))).setCode(resultSet.getString("code")).setName(resultSet.getString("name")).setClosed(Integer.parseInt(resultSet.getString("closed")) == 1).createStatus());
        }
        return statuses;
    }
